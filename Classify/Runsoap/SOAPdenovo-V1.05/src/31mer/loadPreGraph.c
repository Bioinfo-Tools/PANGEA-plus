...
int bisearch ( VERTEX * vts, int num, Kmer target )
{
	...
	while ( low <= high )
	{
		...
		if ( vts[mid].kmer == target )
			{ break; }
		else if ( target > vts[mid].kmer )
			{ low = mid + 1; }
		else
			{ high = mid - 1; }
	}
...
void loadEdge ( char * graphfile )
{
	...
	while ( fgets ( line, sizeof ( line ), fp ) != NULL )
	{
		if ( line[0] == '>' )
		{
			...
			n = 0;
			index++;
			sscanf ( line + 7, "%d,%llx,%llx,%s %d,%d", &length, &from_kmer, &to_kmer, str, &cvg, &bal_ed );
			tightSeq = ( char * ) ckalloc ( ( length / 4 + 1 ) * sizeof ( char ) );
		}
		else
		{
