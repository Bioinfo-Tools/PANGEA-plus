...
static int clipTipFromNode ( kmer_t * node1, int cut_len, boolean THIN )
{
	...
	int sum, count;
	kmer_t * out_node;
	Kmer tempKmer, pre_word, word, bal_word, hash_ban;
	char ch1, ch;
	...
	while ( out_node->linear )
	{
		...
		else
		{
			...
			{
				printf ( "kmer %llx not found, node1 %llx\n", word, node1->seq );
				printf ( "pre_word %llx with %d(larger)\n", reverseComplement ( pre_word, overlaplen ), int_comp ( ch ) );
				exit ( 1 );
			}
		}
	}
	...
	else
	{
		...
		if ( smaller && get_kmer_left_cov ( *out_node, ch ) < max_links )
		{
			...
			if ( count_branch2prev ( out_node ) == 1 &&
			        count_branch2next ( out_node ) == 1 )
				{ out_node->linear = 1; }
