...
static void recordPathBin ( FILE * outfp )
{
	...
	for ( t = 0; t < read_c; t++ )
	{
		...
		if ( finish - start < 3 || mixBuffer[start] == 0
		        || mixBuffer[start + 1] == 0 || mixBuffer[start + 2] == 0 )
			{ continue; }
...
static void search1kmerPlus ( int j, unsigned char thrdID )
{
	...
	{
		/*
		printf("kmerPlus %llx (hashban %llx) not found, flag %d!\n",
		        mixBuffer[j],hashBanBuffer[j],flagArray[j]);
		*/
		mixBuffer[j] = 0;
		return;
	}
...
static void parse1read ( int t, int threadID )
{
	...
	for ( j = start; j < finish; j++ )
	{
		...
		else
		{
			...
			{
				retain++;
				wordplus = KmerPlus ( prevKmer, currentKmer & 3 );
				bal_wordplus = reverseComplementVerbose ( wordplus, overlaplen + 1 );

				if ( wordplus <= bal_wordplus )
				{
					smallerBuffer[pos] = 1;
...
void prlRead2edge ( char * libfile, char * outfile )
{
	...
	edge_no = ( Kmer ** ) ckalloc ( ( thrd_num + 1 ) * sizeof ( Kmer * ) );
	...
	if ( 1 )
	{
		...
		for ( i = 0; i < thrd_num; i++ )
		{
			...
			edge_no[i + 1] = ( Kmer * ) ckalloc ( 2 * maxReadLen * sizeof ( Kmer ) );
	...
	if ( 1 )
	{
		...
		edge_no[0] = ( Kmer * ) ckalloc ( 2 * maxReadLen * sizeof ( Kmer ) );
