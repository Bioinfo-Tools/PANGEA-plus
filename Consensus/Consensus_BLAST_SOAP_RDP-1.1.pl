   ...
   chomp($rdpline[0]);
	...
	$b = 0;
		...
		for ($b = 0; $b < @rdptax; $b = $b+3 ) {
		    ...
		    if ( $blasttax[$a+1] eq $rdptax[$b] and $index1 eq $index2 ) {
			    #|| grep /^$blasttax[$a+1]/, $rdptax[$b]
			    #print OUT "Match: $blasttax[$a+1] and $rdptax[$b]\n";
		    	    $rankmatches++;
	    ...
	    if ( ($blastcount gt $maxblastcount or $blastsim lt $blastline[2]) and $rankmatches eq $maxrankmatches){
	...
	#}
	    ...
	    if( $found eq 0){
		print "not found: $blastline[0]\t $rdpline[0]\n";
		$rankmatches = 0;
		$i++; goto GETBLAST;
		...
		$blastsim = 0;
		$linecount++;
	        #print "$linecount sequences found: $i blast\t $r RDP.\n";
	    }
